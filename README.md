...
| `source_name`   |                   | string  | no  | Name of the specific image source to use when the camera's GetImages response contains multiple images (e.g., ["rgb_1", "rgb_2"]). Video store will always pick the index-0 image to return, so this attribute allows you to set a non-index-0 image source to store e.g. if "rgb_1" always returns first in the images list, setting this to "rgb_2" will save "rgb_2" frames instead. |
