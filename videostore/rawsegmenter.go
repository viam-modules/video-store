...
func (rs *rawSegmenter) initH264(sps, pps []byte) error {
	...
	if ret != C.VIDEO_STORE_RAW_SEG_RESP_OK {
		errors.New("failed to initialize raw segmenter")
	}
	rs.cRawSeg = cRS
	rs.initialized = true
...
func (rs *rawSegmenter) writePacket(payload []byte, pts int64, isIDR bool) error {
	...
	ret := C.video_store_raw_seg_write_h264_packet(rs.cRawSeg, payloadC, pts, isIDR)
	if ret != C.VIDEO_STORE_RAW_SEG_RESP_OK {
		errors.New("failed to write packet")
	}
...
func (rs *rawSegmenter) close() {
	...
	if ret != C.VIDEO_STORE_RAW_SEG_RESP_OK {
		errors.New("failed to close raw segmeneter")
	}
}
