...
int video_store_raw_seg_init_h265(struct raw_seg **ppRS,      // OUT
  ...
  if (rs == NULL) {
    av_log(NULL, AV_LOG_ERROR,
           "video_store_raw_seg_init_h265 failed allocate a raw_seg_h265");
    return VIDEO_STORE_RAW_SEG_RESP_ERROR;
  }
  ...
  if (ret < 0) {
    av_log(
        NULL, AV_LOG_ERROR,
        "video_store_raw_seg_init_h265 failed to allocate format context: %s\n",
        av_err2str(ret));
    goto cleanup;
  }
  /* // Create new stream for the output context. */
  ...
  if (stream == NULL) {
    av_log(NULL, AV_LOG_ERROR,
           "video_store_raw_seg_init_h265 failed to allocate stream");
    ret = VIDEO_STORE_RAW_SEG_RESP_ERROR;
    goto cleanup;
  }
  // NOTE: Nick: Do we need to do this?
  ...
  if (codecCtx == NULL) {
    av_log(NULL, AV_LOG_ERROR,
           "video_store_raw_seg_init_h265 failed to allocate codec context");
    ret = VIDEO_STORE_RAW_SEG_RESP_ERROR;
    goto cleanup;
  }
  ...
  if (codecCtx->extradata == NULL) {
    av_log(NULL, AV_LOG_ERROR,
           "video_store_raw_seg_init_h265 failed to allocate extradata");
    ret = VIDEO_STORE_RAW_SEG_RESP_ERROR;
    goto cleanup;
  }
  codecCtx->extradata_size = (int)extradataSize;
  ...
  if (ret < 0) {
    av_log(NULL, AV_LOG_ERROR,
           "video_store_raw_seg_init_h265 failed to copy codec parameters");
    goto cleanup;
  }
  ...
  if (ret < 0) {
    av_log(NULL, AV_LOG_ERROR,
           "video_store_raw_seg_init_h265 failed to set segment_time");
    goto cleanup;
  }
  ...
  if (ret < 0) {
    av_log(NULL, AV_LOG_ERROR,
           "video_store_raw_seg_init_h265 failed to set segment_format");
    goto cleanup;
  }
  ...
  if (ret < 0) {
    av_log(NULL, AV_LOG_ERROR,
           "video_store_raw_seg_init_h265 failed to set reset_timestamps");
    goto cleanup;
  }
  ...
  if (ret < 0) {
    av_log(NULL, AV_LOG_ERROR,
           "video_store_raw_seg_init_h265 failed to set strftime");
    goto cleanup;
  }
  ...
  if (ret < 0) {
    av_log(NULL, AV_LOG_ERROR,
           "video_store_raw_seg_init_h265 failed to write header");
    goto cleanup;
  }
...
int video_store_raw_seg_write_h265_packet(struct raw_seg *rs,       // IN
  ...
  if (payloadSize == 0) {
    av_log(
        NULL, AV_LOG_ERROR,
        "video_store_raw_seg_write_h265_packet called with empty payload size");
    return VIDEO_STORE_RAW_SEG_RESP_ERROR;
  }

  if (payload == NULL) {
    av_log(NULL, AV_LOG_ERROR,
           "video_store_raw_seg_write_h265_packet called with null payload");
    return VIDEO_STORE_RAW_SEG_RESP_ERROR;
  }

  if (rs->outCtx == NULL) {
    av_log(NULL, AV_LOG_ERROR,
           "video_store_raw_seg_write_h265_packet called before "
           "video_store_raw_seg_write_h265_packet");
    return VIDEO_STORE_RAW_SEG_RESP_ERROR;
  }
  ...
  if (pkt == NULL) {
    av_log(NULL, AV_LOG_ERROR,
           "video_store_raw_seg_write_h265_packet failed to allocate AVPacket");
    ret = VIDEO_STORE_RAW_SEG_RESP_ERROR;
    goto cleanup;
  }
  ...
  if (ret != 0) {
    av_log(NULL, AV_LOG_ERROR,
           "video_store_raw_seg_write_h265_packet failed to create new "
           "AVPacket from data");
    // if av_packet_from_data returned an error then data is not owned by the
    // packet and we need to free it outselves
    av_free(data);
  ...
  if ((ret = av_interleaved_write_frame(rs->outCtx, pkt))) {
    av_log(NULL, AV_LOG_ERROR,
           "video_store_raw_seg_write_h265_packet failed to write frame");
    goto cleanup;
  }
