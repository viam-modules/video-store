...
type RTPVideoStore interface {
	VideoStore
	Init(width, height int, extradata []byte) error
...
func (vs *videostore) Init(width, height int, extradata []byte) error {
	switch vs.typ {
	case SourceTypeH264RTPPacket | SourceTypeH265RTPPacket:
		return vs.rawSegmenter.init(width, height, extradata)
	case SourceTypeFrame:
		fallthrough
	default:
		return errors.New("Init unimplmeented")
	}
}
