...
func (r *renamer) ProcessSegments(ctx context.Context) error {
	...
	r.logger.Info("Starting to watch directory:", r.watchDir)
	...
	for {
		...
		case event, ok := <-watcher.Events:
			...
			if event.Op&fsnotify.Create == fsnotify.Create && strings.HasSuffix(event.Name, ".mp4") {
				r.logger.Info("MP4 file created:", event.Name)
				go r.queueFile(event.Name)
			}
...
func (r *renamer) queueFile(filePath string) {
	...
	r.logger.Info("File queued:", filePath)
...
func (r *renamer) convertFileToUTC(filePath string) error {
	...
	r.logger.Infof("Converting %s to UTC: %s", filename, unixFilename)
...
func (r *renamer) Close() error {
	r.logger.Info("Closing transformer, processing remaining files...")
