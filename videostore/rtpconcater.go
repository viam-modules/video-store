...
func (c *rtpconcater) Concat(from, to time.Time, path string) error {
	...
	for {
		...
		c.logger.Infof("pts: %d, dts: %d, pos: %d, duration: %d, stream_index: %d, time_base: %d, packet.flags: %d", packet.pts, packet.dts, packet.pos, packet.duration, packet.stream_index, packet.time_base, packet.flags)
